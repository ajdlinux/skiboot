...
static void set_power_timer(struct timer *t __unused,
			    void *data,
			    uint64_t now __unused)
{
	...
	case PCI_SLOT_STATE_SPOWER_START:
		if (slot->retries-- == 0) {
			...
			opal_queue_msg(OPAL_MSG_PCI_HOTPLUG, NULL, NULL,
				       slot->power_state, dn->phandle,
				       OPAL_BUSY);
		} else {
	...
	case PCI_SLOT_STATE_SPOWER_DONE:
		if (slot->power_state == OPAL_PCI_SLOT_POWER_OFF) {
			...
			pci_slot_set_state(slot, PCI_SLOT_STATE_NORMAL);
			opal_queue_msg(OPAL_MSG_PCI_HOTPLUG, NULL,
				       NULL, OPAL_PCI_SLOT_POWER_OFF,
				       dn->phandle, OPAL_SUCCESS);
			break;
		}
...
static int64_t opal_pci_set_power_state(uint64_t id, uint64_t data)
{
	...
	if (!slot || !phb)
		return OPAL_PARAMETER;
	if (!slot->ops.get_link_state  ||
	    !slot->ops.get_power_state ||
	    !slot->ops.prepare_link_change)
		return OPAL_UNSUPPORTED;
	...
	if (rc == OPAL_SUCCESS && slot->state == PCI_SLOT_STATE_SPOWER_START) {
		slot->retries = 500;
