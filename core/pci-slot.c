...
static int64_t pci_slot_sm_poll(struct pci_slot *slot, uint8_t *val)
{
	...
	case PCI_SLOT_STATE_GPOWER:
		ret = slot->ops.get_power_status(slot, val);
		break;
			   ...
			   void (*consumed)(void *data))
{
	...
	/*
	 * On unplugging event, the subordinate devices attached
	 * to the PCI slot will be removed. In the mean time, a
	 * signal sent to kernel indicating the completion of the
	 * PCI unplugging.
	 */
	if (!add) {
		pci_remove_bus(phb, &pd->children);
		return opal_queue_msg(OPAL_MSG_PCI_HOTPLUG,
				      slot, consumed,
	...
	/*
	 * On PCI plugging event, we are not able to scan the subordinate
	 * devices attached to the specified PCI slot if the PCIe link is
	 * down. Otherwise, we will hit EEH error without exception.
	 */
	if (slot->ops.get_link_status &&
	    (ret = slot->ops.get_link_status(slot, &link))) {
		prlog(PR_ERR, PCI_SLOT_PREFIX
		      "Error %lld getting link status\n", slot->id, ret);
		link = 0;
	}
